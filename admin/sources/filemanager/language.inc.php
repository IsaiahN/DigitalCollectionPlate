...
if (isset($_POST['custom'])) {
	...
	$exist = $db->select('SELECT * FROM '.$glob['dbprefix'].'CubeCart_lang WHERE `identifier` = '.$db->MySQLSafe($_POST['identifier']));

	if ($exist) {
		$result = $db->update($glob['dbprefix'].'CubeCart_lang', $data, '`identifier` = '.$db->MySQLSafe($_POST['identifier']));
	} else {
		$data['identifier'] = $db->MySQLSafe($_POST['identifier']);
		$result = $db->insert($glob['dbprefix'].'CubeCart_lang', $data);
	}
...
if ($_GET['loc'] == '/') {
	...
	for ($i = 0, $maxi = (count($locArray)-2); $i < $maxi; ++$i) {
		$backLink .= $locArray[$i].'/';
	}
}
...
<?php
	...
	## GET CUSTOM
	$customArrayNat = $db->select('SELECT langArray FROM '.$glob['dbprefix'].'CubeCart_lang WHERE `identifier` = '.$db->mySQLSafe($_GET['loc']));

	if ($customArrayNat && !empty($customArrayNat)) {
