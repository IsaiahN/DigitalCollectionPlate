...
var FCK =
{
	...
	GetData : function( format )
	{
		...
		// ReSite protected attributes.
		data = FCK.ProtectEventsReSite( data ) ;
	...
	ProtectEventsReSite : function( html )
	{
		return html.replace( FCKRegexLib.ProtectedEvents, _FCK_ProtectEvents_ReSiteEvents ) ;
	},
	...
	ProtectTags : function( html )
	{
		...
		//		  positioned. The trick is protect it here and reSite them in
	...
	SwitchEditMode : function( noUndo )
	{
		...
		// Save the current IsDirty state, so we may reSite it after the switch.
...
function _FCK_ProtectEvents_ReSiteEvents( match, encodedOriginal )
{
...
function _FCK_EditingArea_OnLoad()
{
	...
	// ReSite show blocks status.
	FCKCommands.GetCommand( 'ShowBlocks' ).ReSiteState() ;
