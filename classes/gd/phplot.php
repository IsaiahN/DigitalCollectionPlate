...
class PHPlot {
	...
	*/
	...
	private $is_inline		= false;					## FALSE = Sends headers, TRUE = sends just raw image data
	private $browser_cache	= false;					## FALSE = Sends headers for browser to not cache the image,
														## (only if is_inline = FALSE also)
	
	private $safe_margin	= 5;						## Extra margin used in several places. In pixels
	
	private $x_axis_position = '';          // Where to draw both axis (world coordinates),
	private $y_axis_position = '';          // leave blank for X axis at 0 and Y axis at left of plot.
	
	private $xscale_type	= 'linear';					## linear, log
	private $yscale_type	= 'linear';
	
	//Fonts
	private $use_ttf		= false;					## Use TrueType fonts?
	private $ttf_path		= '.';						## Default path to look in for TT Fonts.
	private $default_ttfont = 'benjamingothic.ttf';		## Default TT font file
	private $line_spacing	= 4;						## Pixels between lines.
	
	// Font angles: 0 or 90 degrees for fixed fonts, any for TTF
	private $x_label_angle	= 0;                 // For labels on X axis (tick and data)
	private $y_label_angle	= 0;                 // For labels on Y axis (tick and data)
	
	//Formats
	private $file_format	= 'png';
	private $output_file	= '';                  // For output to a file instead of stdout
	
	//Data
	private $data_type		= 'text-data';           // text-data, data-data-error, data-data, text-data-single
	private $plot_type		= 'linepoints';           // bars, lines, linepoints, area, points, pie, thinbarline, squared
	
	private $label_scale_position = 0.5;        // Shifts data labes in pie charts. 1 = top, 0 = bottom
	private $group_frac_width = 0.7;            // Bars use this fraction (0 to 1) of a group's space
	private $bar_extra_space = 0.5;             // Number of extra bar's worth of space in a group
	private $bar_width_adjust = 1;              // 1 = bars of normal width, must be > 0
	
	private $y_precision	= 1;
	private $x_precision	= 1;
	
	private $data_units_text = '';              // Units text for 'data' labels (i.e: '¤', '$', etc.)
	
	// Titles
	private $title_txt		= '';
	
	private $x_title_txt	= '';
	private $x_title_pos	= 'plotdown';          // plotdown, plotup, both, none
	
	private $y_title_txt	= '';
	private $y_title_pos	= 'plotleft';          // plotleft, plotright, both, none
	...
	## Miscellaneous
	private $callbacks = array(		## Valid callback reasons (see SetCallBack)
		'draw_setup'				=> null,
...
//////////////////////////////////////////////////////
     ...
     */
	public function PHPlot($which_width = 600, $which_height = 400, $which_output_file = null, $which_input_file = null) {
    ...
    /*!
     * Reads an image file. Sites width and height, and returns the image
     * resource. On error, calls PrintError and returns False.
     * This is used by the constructor via SetInputFile, and by tile_img().
     */
	public function GetImage($image_filename, &$width, &$height)
    {
        $error = '';
...
//////////////////////////////////////////////////////////
     ...
     */
	private function FindDataLimits() {
     ...
     *
     *  Note: The margins are calculated, but not Sited, if margins or plot area were
    ...
    function CalcMargins($maximize)
    {
        ...
        // Apply the minimum margins and Site in the object.
     ...
     * This Sites the margins, not the area. That may seem odd, but
    ...
    /*!
     * Sites the desired World Coordinate range of the plot.
     ...
     */
    function SetPlotAreaWorld($xmin=NULL, $ymin=0, $xmax=NULL, $ymax=NULL)
    {
        $this->plot_min_x = $xmin;
        $this->plot_max_x = $xmax;
        $this->plot_min_y = $ymin;
