...
class FileManager {
			...
			case FM_FILETYPE_DL:
				foreach ($_FILES as $file) {
					if (!empty($file['tmp_name']) && $file['error'] == '0' && is_uploaded_file($file['tmp_name'])) {
						...
						$this->_db->insert($this->_config['dbprefix'].'filemanager', $record);
					}
				}
				break;
	...
	public function showFileList($type, $page = 0, $perPage = 50) {
		...
		$sql	= sprintf("SELECT * FROM %sfilemanager WHERE type = '%d' AND disabled = '0' ORDER BY filename ASC %s", $this->_config['dbprefix'], $type, $limit);
	...
	public function buildDatabase($purge = false) {
		...
		if ($fileArray) {
			if ($purge) {
				$this->_db->truncate($this->_config['dbprefix'].'filemanager');
			} else {
				$dupes	= sprintf("SELECT filepath FROM %sfilemanager WHERE 1 ORDER BY filename ASC;", $this->_config['dbprefix']);
			...
			foreach ($fileArray as $key => $file) {
				...
				#	$checksql	= sprintf("SELECT COUNT(`file_id`) as Count FROM %sfilemanager WHERE md5hash = %s AND filepath LIKE '%s%%';", $this->_config['dbprefix'], $record['md5hash'], $target_dir);
					$checksql	= sprintf("SELECT COUNT(`file_id`) as Count FROM %sfilemanager WHERE filepath = %s;", $this->_config['dbprefix'], $record['md5hash'], $record['filepath']);
	...
	public function deleteFile($file_id) {
		$sql	= sprintf("SELECT `filepath` FROM `%sfilemanager` WHERE `file_id` = %d", $this->_config['dbprefix'], $file_id);
		...
		if ($result) {
			...
			$sql = sprintf('DELETE FROM `%sfilemanager` WHERE `file_id` = %d', $this->_config['dbprefix'], $file_id);
			$this->_db->misc($sql);
			## Delete from image idx table
			$idx_path = str_replace("images/uploads/","",$result[0]['filepath']);
		/*	## Update image counts
			$sql	= sprintf("SELECT `photogalleryId` FROM `%simg_idx` WHERE `img` = '%s'", $this->_config['dbprefix'], $idx_path);
			$affectedProducts = $this->_db->select($sql);
			if($affectedProducts){
				foreach($affectedProducts as $row){
					## Set product count for product to n -1
					$sql = sprintf('UPDATE `%sphotogallery` SET `noImages` = `noImages` -1 WHERE `photogalleryId` = %d', $this->_config['dbprefix'], $row['photogalleryId']);
					$this->_db->misc($sql);
				}
			}
			## Remove associated file name from inventory table
			$sql = sprintf("UPDATE `%sphotogallery` SET `image` = '' WHERE `image` = '%s'", $this->_config['dbprefix'], $idx_path);
			$this->_db->misc($sql);*/
			## Remove index from table
			$sql = sprintf("DELETE FROM `%simg_idx` WHERE `img` = '%s'", $this->_config['dbprefix'], $idx_path);
			$this->_db->misc($sql);
			return true;
		}
		return false;
