...
class db {
	...
	private function exec($query) {
		if (isset($GLOBALS['config']['debug']) && (bool)$GLOBALS['config']['debug']) {
	...
	function blocker($user, $level, $time, $login, $loc) {
		...
		$this->delete($glob['dbprefix']."blocker","lastTime<".$expireTime);
		$query = "SELECT * FROM ".$glob['dbprefix']."blocker WHERE `browser` = ".$this->mySQLSafe($_SERVER['HTTP_USER_AGENT'])." AND `ip` = ".$this->mySQLSafe(get_ip_address())." AND `loc`= '".$loc."'";
	...
	function getFulltextIndex($table = 'inventory', $prefix = false) {
		...
		if (is_array($table)) {
			...
			$sql = sprintf("SHOW INDEX FROM %s%s;", $glob['dbprefix'], $table);
			$query = $this->exec($sql);
