...
class admin_session {
	...
	function get_session_data() {
		...
		if (empty($GLOBALS[CC_ADMIN_SESSION_NAME])) {
			...
			$query = sprintf("SELECT * FROM ".$this->glob['dbprefix']."admin_users WHERE sessId = %s", $this->db->mySQLSafe($session_id));
			...
			*/
			...
			if (!(bool)$ccAdminData[0]['isSuper']) {
				$query = sprintf("SELECT %1\$sadmin_sections.sectId, name, `read`, `write`, `edit`, `delete` FROM %1\$sadmin_sections LEFT JOIN %1\$sadmin_permissions ON %1\$sadmin_sections.sectId = %1\$sadmin_permissions.sectId WHERE adminId = %2\$s", $this->glob['dbprefix'], $this->db->mySQLSafe($ccAdminData[0]['adminId']));
	...
	function logout() {
		...
		$this->db->update($this->glob['dbprefix']."admin_users", $record,"sessId = ".$this->db->MySQLSafe($GLOBALS[CC_ADMIN_SESSION_NAME]));
	...
	function createSalt($username,$password) {
		...
		$this->db->update($this->glob['dbprefix']."admin_users", array("password" => $this->db->mySQLSafe($pass_hash),"salt" => $this->db->mySQLSafe($salt)),"username=".$this->db->mySQLSafe($username));
		$this->login($username,$password);
	}
	
	function login($username, $password) {
		$query = "SELECT `adminId`, `salt` FROM ".$this->glob['dbprefix']."admin_users WHERE `username`=".$this->db->mySQLSafe($username);
		$salt = $this->db->select($query);
		
		if($salt[0]['adminId']>0 && empty($salt[0]['salt'])) {
			$query = sprintf("SELECT `adminId` FROM %sadmin_users WHERE `username` = %s AND `password` = %s AND `failLevel` < %s AND `blockTime` < %s", $this->glob['dbprefix'], $this->db->mySQLSafe($username), $this->db->mySQLSafe(md5($password)), $this->ini['bfattempts'], time());
			...
			$query = sprintf("SELECT `adminId` FROM %sadmin_users WHERE `username` = %s AND `password` = %s AND `failLevel` < %s AND `blockTime` < %s", $this->glob['dbprefix'], $this->db->mySQLSafe($username), $this->db->mySQLSafe(md5(md5($salt[0]['salt']).md5($password))), $this->ini['bfattempts'], time());
			$result = $this->db->select($query);
		}
	...
	function createSession($admin_id) {
		...
		$this->db->update($this->glob['dbprefix']."admin_users", $record, "adminId = ".$admin_id);
	}
	...
	*/
	...
	function set_cc_admin_cookie($name, $value) {
		...
		$urlParts = parse_url($GLOBALS['SiteURL']);
